...
export default function Layout({ children }) {
  ...
  return (
    <div
      className="relative min-h-screen w-full text-white overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800 fade-scroll-mask"
    >
      ...
      <header
        ...
        style={{
          ...
          ...(scrolled || forceHeaderShrunk
            ? { backgroundColor: '#1f2937' } // bg-gray-800 fallback
            : {
      ...
      {!hideAnimals && (scrolled || forceHeaderShrunk) && (
        <>
          <div className="hidden lg:flex fixed inset-0 mt-24 items-center justify-between pointer-events-none z-0 transition-opacity duration-1000 opacity-100">
            <img src="/images/taureau.png" alt="Taureau" className="h-auto max-h-[70vh] glow-animal opacity-70" />
            <img src="/images/ours.png" alt="Ours" className="h-auto max-h-[70vh] glow-animal opacity-70" />
          </div>
          <div className="lg:hidden fixed inset-x-0 top-[45vh] flex flex-col items-center gap-16 z-0 pointer-events-none transition-opacity duration-1000 opacity-100">
            <img src="/images/taureau.png" alt="Taureau" className="w-[300px] sm:w-[340px] md:w-[380px] h-auto glow-animal opacity-70" />
            <img src="/images/ours.png" alt="Ours" className="w-[160px] sm:w-[200px] md:w-[240px] h-auto glow-animal opacity-70" />
          </div>
        </>
      )}
      ...
      >
        <nav className="flex flex-col p-6 space-y-4 text-lg">
          ...
          <button onClick={() => setListesOpen(!listesOpen)} className="text-left text-white font-medium focus:outline-none hover:text-teal-300">
            Listes {listesOpen ? '▴' : '▾'}
          </button>
          {listesOpen && (
